window.ThrottleChart = {
    create() {
        ...
        if (allData.length === 0) {
            container.append('div').attr('class', 'no-data').text('No throttle data available for selected lap');
            return;
        }
    ...
    createAxes(g, scales, width, height, margin) {
        ...
        g.append('g').call(d3.axisLeft(scales.yScale).ticks(5));

        g.append('text').attr('transform', 'rotate(-90)').attr('y', 0 - margin.left).attr('x', 0 - (height / 2)).attr('dy', '1em').style('text-anchor', 'middle').style('fill', '#fff').text('Throttle (%)');
    ...
    createLines(g, allData, scales) {
        const line = d3.line().x(d => scales.xScale(d.distance)).y(d => scales.yScale(d.throttle)).curve(d3.curveMonotoneX);
