window.GearChart = {
    create() {
        ...
        // Prepara i dati
        const allData = GearChart.prepareData();

        if (allData.length === 0) {
            container.append('div')
                .attr('class', 'no-data')
                .text('No gear data available for selected lap');
            return;
        }

        // Scale
        const scales = GearChart.createScales(allData, width, height);

        // Assi
        GearChart.createAxes(g, scales, width, height, margin);

        // Linee
        GearChart.createLines(g, allData, scales);

        // Legenda
        GearChart.createLegend(g, allData, width);
    ...
    createAxes(g, scales, width, height, margin) {
        ...
        g.append('g')
            .call(d3.axisLeft(scales.yScale).ticks(8));
